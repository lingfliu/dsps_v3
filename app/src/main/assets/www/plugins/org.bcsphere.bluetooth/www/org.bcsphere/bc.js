...
*/
		...
		var BluetoothFuncs = BC.BluetoothFuncs = function(type){
			if(type == "cordova" && typeof cordova !== "undefined" ){
				...
				this.connect = function(device, success, error){
					var connectSuccess = device.connectSuccess.bind(device,success);
					var connectError = device.connectError.bind(device,error);
					navigator.bluetooth.connectDevice(connectSuccess,connectError,device.deviceAddress,APPURL);
				};
				this.disconnect = function(device, success, error){
					var disconnectSuccess = device.disconnectSuccess.bind(device,success);
					var disconnectError = device.disconnectError.bind(device,error);
					navigator.bluetooth.disconnectDevice(disconnectSuccess,disconnectError,device.deviceAddress,APPURL);
				};

				this.writeCharacteristic = function(character, value, success, error){
					var writeSuccess = character.writeSuccess.bind(character,success);
					var writeError = character.writeError.bind(character,error);
				...
				this.readCharacteristic = function(character, success, error){
					var readSuccess = character.readSuccess.bind(character,success);
					var readError = character.readError.bind(character,error);
				...
				this.subscribe = function(character, success, error){
					var subscribeCallback = character.subscribeCallback.bind(character,success);
					var subscribeError = character.subscribeError.bind(character,error);
					navigator.bluetooth.subscribe(subscribeCallback,subscribeError,character.device.deviceAddress,character.upper.index,character.index);
				};
				this.unsubscribe = function(character, success, error){
					var unsubscribeSuccess = character.unsubscribeSuccess.bind(character,success);
					var unsubscribeError = character.unsubscribeError.bind(character,error);
				...
				this.getRSSI = function(device, success, error){
					var getRSSISuccess = device.getRSSISuccess.bind(device,success);
					var getRSSIError = device.getRSSIError.bind(device,error);
				...
				this.discoverServices = function(device, success, error){
					var discoverServicesSuccess = device.discoverServicesSuccess.bind(device,success);
					var discoverServicesError = device.discoverServicesError.bind(device,error);
					navigator.bluetooth.discoverServices(discoverServicesSuccess,discoverServicesError,device.deviceAddress);
				};
				this.discoverCharacteristics = function(service, success, error){
					var discoverCharacteristicsSuccess = service.discoverCharacteristicsSuccess.bind(service,success);
					var discoverCharacteristicsError = service.discoverCharacteristicsError.bind(service,error);
					navigator.bluetooth.discoverCharacteristics(discoverCharacteristicsSuccess,discoverCharacteristicsError,service.device.deviceAddress,service.index,[]);
				};
				this.discoverDescriptors = function(character, success, error){
					var discoverDescriptorsSuccess = character.discoverDescriptorsSuccess.bind(character,success);
					var discoverDescriptorsError = character.discoverDescriptorsError.bind(character,error);
					navigator.bluetooth.discoverDescriptors(discoverDescriptorsSuccess,discoverDescriptorsError,character.device.deviceAddress,character.upper.index,character.index);
				};
				this.readDescriptor = function(descriptor, success, error){
					var readSuccess = descriptor.readSuccess.bind(descriptor,success);
					var readError = descriptor.readError.bind(descriptor,error);
				...
				this.rfcommConnect = function(uuid,secure,device,success,error){
					var connectSuccess = device.connectSuccess.bind(device,success);
					var connectError = device.connectError.bind(device,error);
					navigator.bluetooth.rfcommConnect(connectSuccess,connectError,device.deviceAddress,APPURL,uuid,secure);
				};
				this.rfcommDisconnect = function(device,success,error){
					var disconnectSuccess = device.disconnectSuccess.bind(device,success);
					var disconnectError = device.disconnectSuccess.bind(device,error);
		...
		var Device = BC.Device = BC.EventDispatcher.extend({
			...
			connect : function(success,error,uuid,secure){
				...
				if(!this.isConnected){
					if(this.type == "BLE"){
						BC.bluetooth.connect(this, success, error);
					}else{
						BC.bluetooth.rfcommConnect(uuid,secure,this,success,error);
					}
				}else{
					success();
				}
			...
			connectSuccess : function(callback){
				this.isConnecting = false;
			...
			connectError : function(callback){
				this.isConnecting = false;
				callback();
			},
			...
			discoverServices : function(success,error){
				...
				if(!this.isDiscovering){
					this.isDiscovering = true;
					BC.bluetooth.discoverServices(this, success, error);
				}
			},

			discoverServicesSuccess : function(callback){
			...
			discoverServicesError : function(callback){
				this.isDiscovering = false;
				callback();
			},
			...
			disconnect : function(success,error){
				if(this.isConnected || this.isConnecting){
					if(this.type == "BLE"){
						BC.bluetooth.disconnect(this, success, error);
					}else if(this.type == "Classical"){
						BC.bluetooth.rfcommDisconnect(this, success, error);
					}
				}else{
					success();
				}
			...
			disconnectSuccess : function(callback){
				this.isConnecting = false;
			...
			disconnectError : function(callback){
				callback();
			},
			...
			getRSSI : function(success,error){
				BC.bluetooth.getRSSI(this, success, error);
			},

			getRSSISuccess : function(callback){
				callback(arguments[1].RSSI);
			},

			getRSSIError : function(callback){
				callback();
			},
	  ...
	  var Service = BC.Service = GATTEntity.extend({
			...
			discoverCharacteristics : function(success,error){
				if(this.characteristics == undefined || this.characteristics == null || this.characteristics.length == 0){
					BC.bluetooth.discoverCharacteristics(this, success, error);
				}else{
					success();
				}
			},

			discoverCharacteristicsSuccess : function(callback){
			...
			discoverCharacteristicsError : function(callback){
				callback();
			},
		...
		var Characteristic = BC.Characteristic = GATTEntity.extend({
			...
			read : function(success,error){
				if(this.property.contains("read")){
					BC.bluetooth.readCharacteristic(this, success, error);
				} else {
					error("This characteristic can't be read, please add 'read' in the property.");
				}
			},
			...
			write : function(type,value,success,error){
				...
				if(this.property.contains("write") || this.property.contains("writeWithoutResponse")){
					BC.bluetooth.writeCharacteristic(this,value,success,error);
				}else{
					error("This characteristic can't be written, please add 'write'/'writeWithoutResponse' in the property.");
				}
			},
			writeSuccess : function(callback){
				callback(arguments);
			},
			writeError : function(callback){
				callback(arguments);
			},
			 ...
			 * @instance
			 */
			subscribe : function(callback, success, error){
				this.callback = callback;
				if(this.property.contains("notify") || this.property.contains("indicate")){
					BC.bluetooth.subscribe(this, success, error);
				}
			},
			subscribeCallback : function(callback){
				// This method is called for both subscribe success and characteristic notifications.
				// The notifications are identified by the presence of the deviceAddress property.
				var obj = arguments[1];
				if (!obj || !obj.deviceAddress) {
			...
			unsubscribe : function(success,error){
				if(this.property.contains("notify")){
					BC.bluetooth.unsubscribe(this, success, error);
				}
			},
			unsubscribeSuccess : function(callback){
				callback();
			},
			...
			discoverDescriptors : function(success,error){
				BC.bluetooth.discoverDescriptors(this, success, error);
			},

			discoverDescriptorsSuccess : function(callback){
			...
			discoverDescriptorsError : function(callback){
				callback();
			},
		...
		var Descriptor = BC.Descriptor = GATTEntity.extend({
			 ...
			 */
			read : function(success,error){
				BC.bluetooth.readDescriptor(this, success, error);
			},
			readSuccess : function(callback){
				var data = {};
				data.deviceAddress=this.device.deviceAddress;
		...
		function onDeviceReady(){
			...
			BC.bluetooth.addSystemListener('disconnect', function(arg){
				...
				BC.bluetooth.devices[arg.deviceAddress].isDiscovering = false;
